    ...
    if ($b.phone -eq $phone -and $b.status -eq "paid") {
        ...
        try {
            ...
            $payload = @{
                ...
                caption = "🎫 Здравствуйте, $firstName!`n`n🎉 Ваш золотой билет на GOLDENMIDDLE готов!`n`n📅 Дата: 26 октября 2025`n⏰ Время: 18:00`n📍 Место: Асман`n🪑 Ваше место: Стол $table, Место $seat`n💵 Цена: 5500 Сом`n🆔 ID билета: $ticketId`n`n📎 Билет во вложении. Покажите его при входе на мероприятие!`n`n🎊 Добро пожаловать на GOLDENMIDDLE!"
            } | ConvertTo-Json
