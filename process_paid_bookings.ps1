# Process paid bookings and send tickets via WhatsApp
...
foreach ($booking in $paidBookings) {
    Write-Output "`n--- Processing Booking ---"
    ...
    try {
        ...
        # Use a public PDF URL for now (since production server files aren't accessible)
        ...
        Write-Output "❌ Error processing booking: $($_.Exception.Message)"
        $errorCount++
    }
}
...
if ($successCount -gt 0) {
    ...
    Write-Output "⚠️ No tickets were sent successfully."
}
